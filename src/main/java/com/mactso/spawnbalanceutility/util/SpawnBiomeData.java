...
public class SpawnBiomeData {
	...
	// Static counters used only for Summary:
...
//	
	...
	public static void balanceBiomeSpawnValues(MinecraftServer server) {
		...
		Registry<Biome> biomeRegistry = getBiomeRegistry(server);


		Field fieldSpawners = SpawnerFieldAccess.get();

		int totalSpawnEntries = 0;
		String mobCategoryString = "";
		...
		for (Biome b : biomeRegistry) {
			...
			int spawnEntriesUsed = 0;

			for (MobCategory mc : MobCategory.values()) {
				List<Weighted<SpawnerData>> newBalancedList = new ArrayList<>();
				mobCategoryString = mc.toString(); // BUG FIX for HybridAquaticFish
				for (BiomeCreatureItem biomeCreatureItem : modBiomeMobSpawners) {
					linenumber++;
					if (biomeCreatureItem.getMobCategory().equalsIgnoreCase(mobCategoryString)) {
						...
						if (optEntityType.isPresent()) {
							if (optEntityType.get().getCategory() == MobCategory.MISC) {
								...
								if (biomeCreatureItem.getModAndMob().equals("minecraft:ocelot")) {
									// the ocelot is a creature but it is in the MONSTER mob category- old minecraft
									// bug.
								} else {
									Utility.debugMsg(0,
							...
							SpawnerData newSpawner = new SpawnerData(optEntityType.get(),
							...
							LOGGER.error("Approx Line#: " + linenumber + "SpawnBalanceUtility ERROR: Mob "
									+ biomeCreatureItem.getModAndMob() + " not in Entity Type Registry");
						}
					}
				}
				// Only add this category if there is at least one configured mob
				if (!newBalancedList.isEmpty()) {
					newMap.put(mc, WeightedList.of(newBalancedList));
					spawnEntriesUsed += newBalancedList.size();
				}
			}
			try {
				@SuppressWarnings("unchecked")
				Map<MobCategory, WeightedList<SpawnerData>> oldMap = (Map<MobCategory, WeightedList<SpawnerData>>) fieldSpawners
						.get(mobSpawnSettings);
				fieldSpawners.set(mobSpawnSettings, newMap);
				totalSpawnEntries += spawnEntriesUsed;
				Summary.biomeUpdate(oldMap, newMap);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		Summary.setBiomeUsed(totalSpawnEntries);
	}
		...
		for (Biome biome : biomeRegistry) {
			...
			for (MobCategory mc : MobCategory.values()) {
				...
				for (Weighted<SpawnerData> wsd : originalSpawnerList.unwrap()) {
					...
					Utility.debugMsg(2, Main.MODID + ":" + wsd.value().type().getDescriptionId()
							+ " minimum Spawn changed from " + oldSpawnWeight + " to " + newSpawnWeight);
...
//					}
				...
				addMassAdditionMobs(mc, biomeHolder, newFixedList, usedList);
	...
	public static void generateBiomeSpawnValuesReport(MinecraftServer server) {
		...
		for (Biome biome : biomeRegistry) {
			...
			// Optional<Holder.Reference<Biome>> oBH =
			// biomeRegistry.getHolder(biomeRegistry.getId(b));
			...
			for (MobCategory mc : MobCategory.values()) {
				for (Weighted<SpawnerData> wsd : msi.getMobs(mc).unwrap()) {
					...
					if (MyConfig.isIncludedMod(modname)) {
						p.println(++lineNumber + ", " + cn + ", " + bn + ", " + mc + ", "
								+ EntityType.getKey(spawnerData.type()).toString() + ", " + wsd.weight() + ", "
								+ wsd.value().minCount() + ", " + wsd.value().maxCount());
					}
				}
			}
	...
	// very local utility methods.
	private static void addMassAdditionMobs(MobCategory mc, Holder<Biome> biomeHolder,
			List<Weighted<SpawnerData>> newFixedList, List<EntityType<?>> usedList) {
		List<MassAdditionMobItem> massAddMobs = MobMassAdditionManager.getFilteredList(mc,
				Utility.getBiomeCategory(biomeHolder));

		for (MassAdditionMobItem ma : massAddMobs) {
			Optional<EntityType<?>> optEntityType = EntityType.byString(ma.getModAndMob());
			if (optEntityType.isEmpty())
				continue;

			EntityType<?> et = optEntityType.get();
			...
			if (!mobFound) {
				SpawnerData newSpawner = new SpawnerData(et, ma.getMinCount(), ma.getMaxCount());
				newFixedList.add(new Weighted<>(newSpawner, ma.getSpawnWeight()));
				usedList.add(et);
			}
		}
	}

	private static Registry<Biome> getBiomeRegistry(MinecraftServer server) {
		Optional<Registry<Biome>> optRegistry = server.registryAccess().lookup(Registries.BIOME);
		if (optRegistry.isEmpty()) {
			LOGGER.error("Hard Error : Biome Registry Missing");
			throw new IllegalStateException("Biome Registry missing");
		}
